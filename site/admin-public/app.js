...
// Combos têm somente um preço: não existe escolha de tamanho P/M/G/F.
const renderPrecosPadrao = renderPrecos;
renderPrecos = function () {
  if (estado.catalogoPrecoAtual === "adicionar") {
    renderPrecosPadrao();
    const seletor = $("#novoTipo");
    if (seletor && !seletor.querySelector('option[value="combo"]')) seletor.insertAdjacentHTML("beforeend", '<option value="combo">Combo</option>');
    return;
  }
  if (estado.catalogoPrecoAtual !== "combos") return renderPrecosPadrao();
  const c = estado.catalogoPrecos || {};
  const itens = Object.entries(c.combos || {}).map(([chave, preco]) => ({ chave, preco, nome: c.nomesCombos?.[chave]?.nome || nomeProdutoCompleto(chave) }));
  $("#precos").innerHTML = '<div class="acao-salvar busca-precos"><button id="salvarPrecosCombos" class="btn primario">Salvar alterações</button></div>' + itens.map(item => '<article class="linha-preco"><strong>🍽️ '+escapar(item.nome)+'</strong><label>Preço do combo <input class="campo-preco" type="number" step=".01" min=".01" value="'+Number(estado.alteracoesPrecos[chavePreco("combo",item.chave)]??item.preco).toFixed(2)+'" data-preco-combo="'+escapar(item.chave)+'"></label></article>').join("");
};
document.addEventListener("input", e => { const campo=e.target.closest("[data-preco-combo]"); if(campo) estado.alteracoesPrecos[chavePreco("combo",campo.dataset.precoCombo)]=campo.value; });
document.addEventListener("click", async e => { const botao=e.target.closest("#salvarPrecosCombos"); if(!botao)return; e.preventDefault(); e.stopImmediatePropagation(); try { botao.disabled=true; for(const [chave,preco] of Object.entries(estado.alteracoesPrecos)){const [tipo,id]=chave.split("|");if(tipo==="combo")await api("/api/painel/precos/combo",{method:"PATCH",body:JSON.stringify({chave:id,preco})});} estado.alteracoesPrecos={}; estado.catalogoPrecos=await api("/api/painel/precos"); renderPrecos(); toast("Preços dos combos salvos."); } catch(erro){toast(erro.message)} finally {botao.disabled=false} }, true);
function atualizarFormularioNovoItem(){const tipo=$("#novoTipo")?.value,bebida=tipo==="bebida",combo=tipo==="combo",bloquearCategoria=bebida||combo,g=$("#grupoCategoria"),c=$("#novaCategoria"),aviso=$("#avisoCategoria"),precosPizza=$("#precosPizzaIniciais"),precoBebida=$("#grupoPrecoBebida"),grupoIngredientes=$("#grupoIngredientesNovo"),campoIngredientes=$("#novoIngredientes");if(!c)return;if(combo&&!c.querySelector('option[value="combos"]'))c.insertAdjacentHTML("beforeend",'<option value="combos">Combos (automático)</option>');c.disabled=bloquearCategoria;if(g)g.classList.toggle("indisponivel",bloquearCategoria);if(aviso){aviso.hidden=!bloquearCategoria;aviso.textContent=combo?"Combos são sempre salvos na categoria Combos.":"Indisponível para bebidas."}if(precosPizza){precosPizza.hidden=bebida||combo;precosPizza.querySelectorAll("input").forEach(x=>x.required=!bebida&&!combo)}if(precoBebida){precoBebida.hidden=!bebida&&!combo;const campo=precoBebida.querySelector("input");if(campo)campo.required=bebida||combo}if(grupoIngredientes)grupoIngredientes.hidden=false;if(campoIngredientes){campoIngredientes.required=true;campoIngredientes.placeholder=combo?"Ex.: 1 pizza grande, refrigerante e sobremesa.":bebida?"Ex.: Refrigerante gelado, 2 litros.":"Ex.: Mussarela, calabresa e orégano."}if(combo)c.value="combos";else if(bebida)c.value="tradicionais"}
document.addEventListener("submit",e=>{if(e.target.id==="formNovoItem"&&$("#novoTipo")?.value==="combo")$("#novaCategoria").value="combos"},true);
...
// Combos usam descrição própria, e não ingredientes de pizza.
